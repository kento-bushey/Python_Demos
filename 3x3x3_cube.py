...
for i in range(rubix_size):
    for j in range(rubix_size):
        for k in range(rubix_size):
            size = 35
            t_r_offset = 40
            val_offset = (rubix_size-1)/2
            i_val = (i-val_offset)*t_r_offset
            j_val = (j-val_offset)*t_r_offset
            k_val = (k-val_offset)*t_r_offset
            box = rectangularPrism([i_val,j_val,k_val],size,size,size,(255,0,0))
            scene.append(box)
